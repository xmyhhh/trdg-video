...
class FakeTextDataGenerator(object):
    ...
    def generate(
        ...
        if alignment == 0 or width == -1:
            ...
            import random
            x0=random.randint(0,background_img.size[0]-resized_img.size[0])
            y0=random.randint(0,background_img.size[1]-resized_img.size[1])
            background_img.paste(resized_img, (x0, y0), resized_img)
            background_mask.paste(resized_mask, (x0, y0))
        ...
        if out_dir is not None:
         ...
         #save gt
        import utils
        utils.text_create("out/"+image_name+".txt",
                          str(x0)+","+str(y0)+","+str(x0+resized_img.size[0])+","+str(y0+resized_img.size[1])
                                                    )